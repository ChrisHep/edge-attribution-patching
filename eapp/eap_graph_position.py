...
class EAPGraph:
    ...
    def __init__(self, cfg, upstream_nodes=None, downstream_nodes=None, edges=None):
        ...
        # adjustment to include scores per position
        self.eap_scores_per_position: Float[Tensor, "n_upstream_nodes n_downstream_nodes"] = None
    ...
    def reset_scores(self):
        self.eap_scores_per_position = torch.zeros(
    ...
    def top_edges(
        ...
        assert self.eap_scores_per_position is not None, "EAP scores have not been computed yet"       

        # get indices of maximum values in 2d tensor
        if abs_scores:
            top_scores, top_indices = torch.topk(self.eap_scores_per_position.flatten().abs(), k=n, dim=0)
        else:
            top_scores, top_indices = torch.topk(self.eap_scores_per_position.flatten(), k=n, dim=0)
        ...
        for i, (abs_score, index) in enumerate(zip(top_scores, top_indices)):
            ...
            upstream_node_idx, downstream_node_idx = np.unravel_index(index, self.eap_scores_per_position.shape)
            score = self.eap_scores_per_position[upstream_node_idx, downstream_node_idx]
