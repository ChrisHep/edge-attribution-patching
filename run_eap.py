...
def run_exp(args: DictConfig):
    ...
    wandb_name = f"{args.model}_{args.n_shots}_shots_max_{args.max_n}_{args.representation}"
    ...
    clean_logits = logits_in_batches(model, intervention_data.alt_string_toks, intervention_data.alt_attention_mask, 9)
    corrupt_logits = logits_in_batches(model, intervention_data.base_string_toks, intervention_data.base_attention_mask, 9)
    ...
    print(f"clean logit diff: {clean_logit_diff}, corrupt logit diff: {corrupt_logit_diff}")
    ...
    graph = EAP(
        model,
        clean_tokens=intervention_data.alt_string_toks,
        corrupted_tokens=intervention_data.base_string_toks,
        metric=metric,
    ...
    top_edges = graph.top_edges(n=k_edges, abs_scores=True)
